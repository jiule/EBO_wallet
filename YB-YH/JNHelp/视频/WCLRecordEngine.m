...
#pragma clang diagnostic pop
    if (iOS10) {
        AVCaptureDeviceDiscoverySession * session =  [AVCaptureDeviceDiscoverySession alloc];
...
- (void)openFlashLight {
    ...
    if (backCamera.torchMode == AVCaptureTorchModeOff) {
        ...
        if (iOS10) {
            AVCapturePhotoSettings * bactSet = [AVCapturePhotoSettings photoSettings] ;
            bactSet.flashMode = AVCaptureFlashModeOn;
        }else {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"      //方法弃用
...
- (NSString *)getVideoCachePath {
    NSString *videoCache = [FileManager videoPath];
    BOOL isDir = NO;
